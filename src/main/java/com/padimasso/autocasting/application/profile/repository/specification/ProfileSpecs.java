package com.padimasso.autocasting.application.profile.repository.specification;
...
public final class ProfileSpecs {
    ...
    public static Specification<ProfileEntity> hasHeadshot() {
        return (root, q, cb) -> {
    ...
    public static Specification<ProfileEntity> stageNameContains(String text) {
    ...
    public static Specification<ProfileEntity> genderInTokens(List<String> tokens) {
    ...
    public static Specification<ProfileEntity> ageBetween(Integer min, Integer max) {
    ...
    // ManyToMany: ALL
    public static Specification<ProfileEntity> allOf(String relationPath, List<UUID> ids) {
        if (ids == null || ids.isEmpty()) return null;
        return (root, q, cb) -> {
            var sq = q.subquery(Long.class);
            var p2 = sq.from(ProfileEntity.class);
            From<?, ?> from = p2;
            for (var part : relationPath.split("\\.")) {
                from = from.join(part, JoinType.INNER);
            }
