-- Seed consolidado de metadata/legal basado en MVP 1, con estado final
-- equivalente a las migraciones históricas posteriores.

------------------------------------------------------------------------
---- Insert Roles (estado final)
------------------------------------------------------------------------
INSERT INTO roles (
  id,
  code,
...
VALUES
(gen_random_uuid(), 'TALENT', 'role.talent', 'Usuario que representa un talento', NOW(), 'SYSTEM', NOW(), 'SYSTEM', false),
(gen_random_uuid(), 'EMPLOYER', 'role.employer', 'Usuario que busca y gestiona actores', NOW(), 'SYSTEM', NOW(), 'SYSTEM', false)
ON CONFLICT (code) DO UPDATE SET
  name_string_code = EXCLUDED.name_string_code,
  description = EXCLUDED.description,
  deleted = false,
  modified_at = NOW(),
  modified_by = 'FLYWAY_V31';
...
VALUES
    ...
    (gen_random_uuid(), 'sitemetadata.gender.indistinct', NOW(), 'SYSTEM', NOW(), 'SYSTEM', false);
...
--------------------------------
-- Historical Fixes Applied In Seed
--------------------------------

-- V21 + V22 + V23: transición de acentos a spanish_mx y alineación de timestamps.
UPDATE skills
SET
    deleted = true,
    modified_at = NOW(),
...
-- V24 + V25 + V26: asegurar gender indistinct como canonical.
...
UPDATE public.gender_option
SET
    string_code = 'sitemetadata.gender.indistinct',
    created_at = (
    ...
    modified_at = NOW(),
    modified_by = 'SYSTEM'
WHERE string_code = 'sitemetadata.gender.other'
  AND NOT EXISTS (
      SELECT 1
...
-- V27: asegurar pay_rate_type collaborative.
...
WHERE NOT EXISTS (
    ...
    WHERE string_code = 'sitemetadata.pay_rate_type.collaborative'
);
